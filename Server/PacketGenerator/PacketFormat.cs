...
{{
    ...
    {{
		...
		pos += sizeof(ushort);
		Array.Copy(BitConverter.GetBytes((ushort)PacketID.{0}), 0, segment.Array, segment.Offset + pos, sizeof(ushort));
		pos += sizeof(ushort);
		{3}
...
@"this.{0} = BitConverter.{1}(segment.Array, segment.Offset + pos);
pos += sizeof({2});
";
...
@"ushort {0}Leng = BitConverter.ToUInt16(segment.Array, segment.Offset + pos);
pos += sizeof(ushort);
this.{0} = Encoding.Unicode.GetString(segment.Array, segment.Offset + pos, {0}Leng);
pos += {0}Leng;
";
...
@"this.{1}s.Clear();
ushort {1}Leng = BitConverter.ToUInt16(segment.Array, segment.Offset + pos);
pos += sizeof(ushort);			
...
@"
Array.Copy(BitConverter.GetBytes({0}), 0, segment.Array, segment.Offset + pos, sizeof({1}));
pos += sizeof({1});
";
...
Array.Copy(BitConverter.GetBytes({0}Leng), 0, segment.Array, segment.Offset + pos, sizeof(ushort));
pos += sizeof(ushort);
pos += {0}Leng;
";
		// {0} 리스트 이름 [대문자]
...
@"
Array.Copy(BitConverter.GetBytes((ushort)this.{1}s.Count), 0, segment.Array, segment.Offset + pos, sizeof(ushort));
pos += sizeof(ushort);

foreach({0} {1} in this.{1}s)
	{1}.Wrtie(segment, ref pos);
";
